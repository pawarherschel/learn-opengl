...
#ifndef SHADER_H  // NOLINT(llvm-header-guard)
#define SHADER_H
...
inline shader_program::shader_program(const uint32_t shader_id): shader_id_{
    shader_id} {
}
...
inline auto
shader_program::set_bool(const std::string &name,
                         const bool value) const -> void {
  glUniform1i(glGetUniformLocation(this->shader_id_, name.c_str()),
              static_cast<GLint>(value));
}

inline auto shader_program::set_int(const std::string &name,
                                    const int value) const -> void {
  glUniform1i(glGetUniformLocation(this->shader_id_, name.c_str()), value);
}

inline auto shader_program::set_float(const std::string &name,
                                      const float value) const -> void {
  glUniform1f(glGetUniformLocation(this->shader_id_, name.c_str()),
              value);
}
...
public:
  explicit program_builder(): program_id_{glCreateProgram()} {
  }

  auto add_shader(uint32_t shader_type,
...
inline auto shader_program::builder::program_builder::add_shader(
  ...
  * {
  ...
  if (0 == success) {
    glGetShaderInfoLog(shader_id, info_log_buffer_size, nullptr,
                       info_log.data());
  ...
  if (0 == success) {
    glGetProgramInfoLog(this->program_id_, info_log_buffer_size, nullptr,
                        info_log.data());
...
inline auto shader_program::builder::program_builder::set_bool(
    ...
    const bool value) const -> void {
  glUniform1i(glGetUniformLocation(this->program_id_, name.c_str()),
              static_cast<GLint>(value));
}

inline auto shader_program::builder::program_builder::
set_int(const std::string &name, const int value) const -> void {
...
inline auto
shader_program::builder::program_builder::build() const -> shader_program {
  assert(!this->err_);
